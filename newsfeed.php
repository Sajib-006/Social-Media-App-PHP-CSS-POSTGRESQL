...
<body>
	<?php
		...
		else if($_REQUEST['liked']=='0')
		{
			...
			$r2=pg_fetch_array($r);
			echo"hi",gettype($r2[0]),$r2[0];
			if($r2[0]==NULL) pg_query("Insert into like_table(like_id,liker_id,p_id,c_id,like_unlike) Values(default,$u,$y,NULL,0)");
			else pg_query("Update like_table set like_unlike=0 where p_id=$y AND liker_id=$u");
		}
						...
						echo "<td>";
							...
							/*if(gettype($_REQUEST['liked'] )==NULL)
							   ...
							   echo"<a href='insert_like.php?pid=$post_id&liked=0' class='button'>Unlike</a>";
								...
								echo"<a href='insert_like.php?pid=$post_id&liked=1' class='button'>Like</a>";
							else echo"<a href='insert_like.php?pid=$post_id&liked=1' class='button'>Like</a>";*/
